...
const { convertHourLimit, convertDayLimit } = require('../middleware/rateLimiter');
...
router.post(
  ...
  async (req, res) => {
    ...
    if (text.length > 20000)
      return res.status(400).json({ error: '文档内容不能超过 20000 字' });
    ...
    try {
      ...
      if (!isGuest) {
        ...
        db.completeTask(req.user.id, mode === 'official' ? 'first_official' : 'first_convert');
      }
